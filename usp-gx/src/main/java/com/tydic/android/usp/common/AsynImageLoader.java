...
import com.tydic.android.usp.util.PictureUtil;
...
public class AsynImageLoader {
    ...
    public void showImageAsyn(ImageView imageView, String url, int resId){  
        ...
        imageView.setTag(R.id.tag_asynimage_stat, "failure"); //下载状态
        ...
        if(bitmap == null){  
            ...
            imageView.setTag(R.id.tag_asynimage_stat, "success"); //下载完成
        }  
    }  
      
    //只从cache取图片
    public Bitmap loadImageFromCache(String path){  
        // 判断缓存中是否已经存在该图片  
    ...
    //从缓存删除图片
    public void removeImageFromCache(String path){  
        // 判断缓存中是否已经存在该图片  
        if(caches.containsKey(path)){  
            caches.remove(path);
        }
        ...
        return new ImageCallback() {  
            ...
            public void loadImage(String path, Bitmap bitmap) {  
                if(path.equals(imageView.getTag().toString())){  
                    ...
                    imageView.setImageResource(resId);
                    imageView.setTag(R.id.tag_asynimage_stat, "failure");
                }  
            }  
        };  
    }  
        ...
        public void run() {  
            while(isRunning){  
                ...
                while(taskQueue.size() > 0){  
                    ...
                    // 将下载的图片添加到缓存  
                    task.bitmap = PictureUtil.getbitmap(task.path);  
